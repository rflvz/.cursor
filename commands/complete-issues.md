...
## ALERTA CRÍTICA - DOS PASOS QUE NUNCA DEBES OLVIDAR

**ANTES DE CONTINUAR, LEE ESTO:**

1. **CREAR PULL REQUEST CON GH CLI:**
   - Después de hacer push a tu branch, SIEMPRE ejecuta: `gh pr create`
   - NO uses la interfaz web, usa gh cli
   - NO continúes sin crear el PR
   - Este paso es OBLIGATORIO cuando trabajas con Git

2. **MOVER ISSUE A "IN REVIEW":**
   - Inmediatamente después de crear el PR, DEBES mover el issue a "In Review"
   - Usa: `update_issue` con `state: "In Review"`
   - Este paso es OBLIGATORIO y NO puede omitirse
   - NO termines la implementación sin mover a "In Review"

**Estos dos pasos son los que más se olvidan. NO los olvides.**
...
**SIEMPRE trabaja con branches y Pull Requests usando gh cli. NUNCA commits directos a `main` o `master`.**
**OBLIGATORIO: Crear PR con `gh pr create` antes de mover a "In Review".**
...
# 5. Crear Pull Request con gh cli (OBLIGATORIO)
gh pr create --title "feat: Implementación para ISSUE-123" --body "Closes #ISSUE-123" --base main
```
...
**OBLIGATORIO: Este paso DEBE completarse antes de pasar a "In Review".**
...
**CREAR PULL REQUEST CON GH CLI (OBLIGATORIO):**

**NO PUEDES CONTINUAR AL PASO 6 SIN HABER CREADO EL PR CON GH CLI.**
...
# 2. Verificar que los cambios están pusheados
git status

# 3. CREAR PULL REQUEST CON GH CLI (OBLIGATORIO)
gh pr create --title "feat: Implementación para ISSUE-123" --body "Closes #ISSUE-123
...
- [ ] Sin regresiones" --base main

# 4. Obtener URL del PR para mencionarlo en Linear
gh pr view --web
```

**IMPORTANTE:**
- **SIEMPRE** crear PR cuando trabajes con Git
- **SIEMPRE** usar `gh pr create` (NO usar la interfaz web)
- Enlazar con issue: `Closes #ISSUE-123` en el body del PR
- Guardar la URL del PR para mencionarla en el comentario de Linear
- **NO continuar al PASO 6 hasta que el PR esté creado**

### PASO 6: Mover a "In Review"

**CRÍTICO: Este paso es OBLIGATORIO. NO puedes omitirlo. DEBES mover el issue a "In Review" inmediatamente después de completar la implementación y crear el PR.**

**CHECKLIST PRE-REVIEW (Todos deben cumplirse ANTES de mover a "In Review"):**

- [ ] Código implementado completamente
- [ ] Tests pasan (si aplica)
- [ ] Funciona correctamente
- [ ] Commiteado y pusheado a branch
- [ ] **Pull Request creado con gh cli (VERIFICAR)**
- [ ] URL del PR obtenida
- [ ] Trabajo documentado

**PROCESO OBLIGATORIO DE ACTUALIZACIÓN A "IN REVIEW":**

**PASO 6.1: ACTUALIZAR ESTADO A "IN REVIEW" (OBLIGATORIO - PRIMERO)**

**NO puedes continuar sin hacer esto. Es el paso más importante después de la implementación.**
...
**Este debe ser el PRIMER paso después de crear el PR. Sin excepciones.**

**PASO 6.2: Añadir comentario profesional:**
...
Parámetros: {
  ...
  ### Pull Request:
  [URL completa del PR obtenida con gh pr view]
...
- Si trabajaste con Git, DEBES haber creado el PR con `gh pr create` antes de este paso
- Si NO trabajaste con Git, puedes omitir la sección del PR pero DEBES mover a "In Review"
- **NUNCA omitas mover el issue a "In Review" después de completar la implementación**
...
- [ ] **Issue movido a "In Review" (OBLIGATORIO - NO OMITIR)**
...
8. **CREAR PR CON GH CLI:** Siempre usa `gh pr create` después de hacer push
9. **MOVER A "IN REVIEW":** Siempre mueve el issue a "In Review" después de crear el PR
...
- Saltarte la creación de Pull Request con gh cli (si trabajas con Git)
- Omitir mover el issue a "In Review" después de completar la implementación
- Implementar sin entender completamente el issue

**SIEMPRE haz esto:**
- Seguir los 8 pasos en orden
- Crear branch para cada issue
- Mover a "In Progress" → implementar → **Crear PR con gh cli** → **Mover a "In Review" (OBLIGATORIO)** → esperar aprobación → "Done"
- Crear Pull Request con `gh pr create` cuando trabajes con Git
- Mover el issue a "In Review" inmediatamente después de crear el PR
...
**RECORDATORIO FINAL CRÍTICO - NO OLVIDES:**
1. Después de implementar y hacer push: `gh pr create` (si trabajas con Git)
2. Inmediatamente después: `update_issue` con `state: "In Review"`
3. Estos dos pasos son OBLIGATORIOS. NO los omitas.
...
**SIEMPRE branches y Pull Requests usando gh cli. NUNCA commits a `main` o `master`.**
...
**PASO 5:** Commits profesionales → Push a branch → **Crear Pull Request con gh cli (OBLIGATORIO)**

**PASO 6:** Checklist pre-review → **Mover a "In Review" (OBLIGATORIO - NO OMITIR)** → Comentario con firma
...
```
**Tomate un respiro y piensa paso a paso**
**NUNCA:**
...
- Crear Pull Request con gh cli cuando trabajes con Git
- **Mover a "In Review" cuando termines la implementación (OBLIGATORIO - NO OMITIR)**
...
- Marcar "Done" sin aprobación
- Omitir firma en comentarios
- Omitir crear PR con gh cli (si trabajas con Git)
- Omitir mover a "In Review" después de completar la implementación
...
1. Después de implementar: Crear PR con `gh pr create` (si trabajas con Git)
2. Inmediatamente después: Mover issue a "In Review" con `update_issue`
3. Estos dos pasos son OBLIGATORIOS y NO pueden omitirse
